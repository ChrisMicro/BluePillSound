...
uint8_t Talkie::pgm_read_byte1(uint8_t *address)
{
  return *address;
}
...
uint8_t Talkie::getBits(uint8_t bits) {
  uint8_t value;
  uint16_t data;
  data = rev(pgm_read_byte1(ptrAddr)) << 8;
  if (ptrBit + bits > 8) {
    data |= rev(pgm_read_byte1(ptrAddr + 1));
  }
  data <<= ptrBit;
  value = data >> (16 - bits);
  ptrBit += bits;
...
void Talkie::say(uint8_t* addr) {
  ...
  do {
    ...
    for(k=0;k<FS/25;k++)
    {
      if( talkieWaveBufferIndex < talkieWaveBufferLength )
      {
        talkieWaveBuffer[talkieWaveBufferIndex++]=nextSample();
      }
